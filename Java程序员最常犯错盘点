...
我们知道，ArrayList的构造方法可接受一个Collection类型的对象，而我们的java.util.Arrays.ArrayList正好也是它的一个子类。实际上，更高效的代码示例是：
...
这个示例代码的输出结果是：
[b,d]
这个示例代码中存在一个非常严重的错误。当一个元素被移除时，该List的大小(size)就会缩减，同事也改变了索引的指向。所以，在迭代的过程中使用索引，将无法从List中
中正确的删除多个指定的元素.
你可能知道解决这个错误的方式之一是使用迭代器(iterator)。而且，你可能认为Java中的foreach语句与迭代器(iterator)是非常相似的，但实际情况并不是这样。看一下如下代码示例：
ArrayList<String> list=new ArrayList<String>(Arrays.asList("a","b","c","d"));
for(String s : list){
...
学习过数据结构的读者都知道一种非常重要的数据结构叫做哈希表。在Java中，对应哈希表的类是HashMap而不是Hashtable.
HashMap和Hashtable之间最大的核心区别就是:HashMap是非同步的，Hashtable是同步的。

5、ArrayList vs LinkedList
喝多Java的初学者不明白ArrayList与LinkedList之间的区别，所以他们完全只用相对简单的ArrayList,甚至不知道JDK中还存在LinkedList.但是，在某些具体场景下，这两种List
的选择会导致程序性能的巨大差异。简单而言：当应用场景中有很多的add/remove操作，只有少量的随机访问操作时，应该选择LinkedList;其他场景下，考虑使用ArrayList.
