...
singleTop:顶单例模式（栈顶复用模式），这种模式和standard模式基本相似，但有一点不同，当要被启动的目标activity已经位于栈顶时，系统不会重新创建目标
...
singleInstance：全局单例模式，这种加载模式，系统保证无论从那个任务栈中启动目标activity，只会创建一个目标activity实例，并会使用一个全新的的任务栈 
来装载该activity实例。          
